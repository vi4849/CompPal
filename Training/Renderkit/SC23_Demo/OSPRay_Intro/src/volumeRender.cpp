...
int main(int argc, const char **argv) {
  ...
  float cam_pos[] = {0.f, 0.f, -20.f}; // LETS MOVE BACK A BIT
  ...
  // ############ NEW STEP DEFINE A FIELD #################
  int DIM = 100;
  float *values = new float[DIM*DIM*DIM];
  for (int i = 0; i < DIM; i++) {
    float fi = 0.5f - (float)i/DIM;
    fi = fi * fi;
    for (int j = 0; j < DIM; j++) {
      float fj = 0.5f - (float)j/DIM;
      fj = fj * fj;
      for (int k = 0; k < DIM; k++) {
        float fk = 0.5f - (float)k/DIM;
        fk = fk * fk;
        values[i*DIM*DIM+j*DIM+k] = powf(fi+fj+fk, 0.5); //distance to center
      }
    }
  }

  // ############ NEW STEP DEFINE OSPRAY HANDLES TO FIELD #################
    OSPData valuesD = ospNewSharedData3D(values, OSP_FLOAT, DIM, DIM, DIM);
  ospCommit(valuesD);

  OSPVolume volume = ospNewVolume("structuredRegular");
  ospSetObject(volume, "data", valuesD);
  float gridSpacing[] = {3.f/DIM, 3.f/DIM, 3.f/DIM};
  ospSetParam(volume, "gridSpacing", OSP_VEC3F, gridSpacing);
  ospCommit(volume);
  ospRelease(valuesD);

  // ############ NEW STEP DEFINE VALUE TO APPEARANCE MAP
  ...
  float lutOpacity[] = {
    1.0f,
    0.0f};
  OSPData lutColorD = ospNewSharedData1D(lutColor, OSP_VEC3F, 2);
  ospCommit(lutColorD);
  OSPData lutOpacityD = ospNewSharedData1D(lutOpacity, OSP_FLOAT, 2);
  ospCommit(lutOpacityD);

  float vrange[] = { 0.f, 1.f };
  ospSetParam(tf, "value", OSP_BOX1F, vrange);
  ospSetObject(tf, "color", lutColorD);
  ospSetObject(tf, "opacity", lutOpacityD);
  ospCommit(tf);
  ospRelease(lutColorD);
  ospRelease(lutOpacityD);

  OSPVolumetricModel volmodel = ospNewVolumetricModel(volume);
  ospSetObject(volmodel, "transferFunction", tf);
  ospCommit(volmodel);
  ospRelease(volume);
  ospRelease(tf);
  ...
  OSPMaterial mat = ospNewMaterial("scivis", "obj");
  ospCommit(mat);
  ...
  ospSetObjectAsData(group, "volume", OSP_VOLUMETRIC_MODEL, volmodel); // SHOW VOLUME
  ospCommit(group);
  ospRelease(model);
  ospRelease(volmodel);
  ...
  OSPRenderer renderer = ospNewRenderer("scivis");  // choose between pathtracer or scivis
  ...
  printf("rendering initial frame to volumeRender.png...");
  ...
  writePNG("volumeRender.png", imgSize, fb);
