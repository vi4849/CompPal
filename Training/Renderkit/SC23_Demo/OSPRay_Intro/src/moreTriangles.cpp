...
int main(int argc, const char **argv) {
  ...
  // ########### ADDED 3 verts and 1 triangle HERE
  // 7 vertices this time
  float vertex[] = {
    -1.0f, -1.0f, 3.0f, 
    -1.0f, 1.0f, 3.0f,
    1.0f, -1.0f, 3.0f, 
  ...
  // release data
  // commit mesh
  OSPData data = ospNewSharedData1D(vertex, OSP_VEC3F, 7); //Bigger NOW
  ospCommit(data);
  ...
  data = ospNewSharedData1D(color, OSP_VEC4F, 7); //Bigger NOW
  ospCommit(data);
  ospSetObject(mesh, "vertex.color", data);
  ospRelease(data);

  // repeat for index instead of color
  data = ospNewSharedData1D(index, OSP_VEC3UI, 3); //Bigger NOW
  ospCommit(data);
  ...
  printf("rendering initial frame to moreTriangles.png...");
  ...
  writePNG("moreTriangles.png", imgSize, fb);
