...
int main(int argc, char **argv)
{
  ...
  // a dense 3D array.
  VKLVolume volume = vklNewVolume(device, "structuredRegular");    // NEW

  // We have to set a few parameters on the volume.
  // First, Open VKL needs to know the extent of the volume:
  vklSetVec3i(volume, "dimensions", res, res, res);                // NEW

  // By default, the volume assumes a voxel size of 1. Scale it so the
  // domain is [0, 1].
  const float spacing = 1.f / static_cast<float>(res);             // NEW
  vklSetVec3f(volume, "gridSpacing", spacing, spacing, spacing);   // NEW
  ...
  // the voxels.
  VKLData voxelData = vklNewData(                                  // NEW
  ...
  vklSetData(volume, "data", voxelData);                           // NEW
  vklRelease(voxelData);                                           // NEW

  // Finally, commit. This may build acceleration structures, etc.
  vklCommit(volume);                                               // NEW
  ...
  vklRelease(sampler);                                             // NEW
  vklRelease(volume);                                              // NEW
