...
void fnct_castRay(RTCScene my_scene, float ox, float oy, float oz, float dx, float dy,
  ...
  // we create the RTCRayHit structure named rayhit and holds the ray and the hit
  ...
  if (rayhit.hit.geomID != RTC_INVALID_GEOMETRY_ID) {
    ...
    /* find the dot product of the normal to the cast */
    ...
    printf("x %f, y %f, z %f\n", nextX, nextY, nextZ);
    printf("shadow cast from just above the surface through the surface again should be a hit (in-shadow)\n");
    ...
    printf("x %f, y %f, z %f\n", nextX, nextY, nextZ);
    printf("shadow cast from just above the surface away from the surface should be a miss (in-light)\n");
    rayhit.ray.org_x = nextX;
    rayhit.ray.org_y = nextY;
    rayhit.ray.org_z = nextZ;
    rayhit.ray.dir_x = 0.f;
...
int main() {
  ...
  // Note that the direccion of both rays is the same (0, 0, 1)
